...
plot(cldRQ)
RQS=summaryBy(RQ~Sex+Treatment+Stock,data=resp,FUN = mean,na.rm=T, stringsAsFactors = FALSE)
...
O2 <- Anova(res.aov2)
O2
O2_1 <- emmeans(res.aov2, ~Sex*Treatment*Stock)
plot(O2_1)
cldO2 <-cld(O2_1, Letters = letters, alpha = 0.05)
O2S=summaryBy(O2pHr2~Treatment+Sex+Stock,data=resp,FUN = mean,na.rm=T, stringsAsFactors = FALSE)
...
CO2 <- Anova(res.aov4)
CO2
CO2_1 <- emmeans(res.aov4, ~Sex*Treatment*Stock)
plot(CO2_1)
cldCO2 <-cld(CO2_1, Letters = letters, alpha = 0.05)
CO2S=summaryBy(CO2pHr2~Treatment+Sex+Stock,data=resp,FUN = mean,na.rm=T, stringsAsFactors = FALSE)
